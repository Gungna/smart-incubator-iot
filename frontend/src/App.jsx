...
// Untuk development: gunakan proxy dari vite.config.js (/api)
// Untuk production: set VITE_API_URL environment variable
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isLogin) {
      ...
      if (username.trim().length > 50) {
        toast.error("Username maksimal 50 karakter");
        return;
      }
      ...
      // Bcrypt memiliki batasan 72 bytes
      const passwordBytes = new TextEncoder().encode(password).length;
      if (passwordBytes > 72) {
        toast.error("Password maksimal 72 karakter (batasan keamanan)");
        return;
      }
    }
    ...
    try {
      if (isLogin) {
        ...
        toast.success(res.data.msg || "Registrasi Sukses! Silakan login.", { id: loadingToast });
        setIsLogin(true);
        setUsername("");
        setPassword("");
      }
      ...
      const errorMsg = err.response?.data?.detail || err.message || "Gagal Masuk/Daftar";
